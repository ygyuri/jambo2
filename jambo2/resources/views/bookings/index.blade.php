            ...
            @if($bookings->count() > 0)
                @foreach($bookings as $booking)
                    <tr>
                        ...
                        <td class="align-middle">{{ $booking->booking_date->format('Y-m-d') }}</td>
                        <td class="align-middle">{{ $booking->booking_reference }}</td>
                        <td class="align-middle">{{ $booking->notes }}</td>
                        <td class="align-middle">
                            <div class="btn-group" role="group" aria-label="Basic example">
                                <a href="{{ route('bookings.show', $booking->id) }}" type="button" class="btn btn-secondary">Detail</a>
                                <a href="{{ route('bookings.edit', $booking->id) }}" type="button" class="btn btn-warning">Edit</a>
                                <form action="{{ route('bookings.destroy', $booking->id) }}" method="POST" onsubmit="return confirm('Delete?')">
                                    @csrf
                                    @method('DELETE')
